...
FR-RBAC-01: Dynamic RBAC per tenant — roles are data stored in DB per tenant, not global enums. Each tenant defines role names (free text) and permission sets per module. Template sectoral roles are the starting point. Guards check permission codes (catalog.edit, session.open) not role names. Architecture H1 — API Phase 2b — AI RBAC Phase 2c. Evolution toward ABAC (conditions: department, amount, document state) documented in FR-RBAC-01 ABAC roadmap — Phase 3+.

**Document Workflow Engine (FR-WORKFLOW-01 to FR-WORKFLOW-02)**
FR-WORKFLOW-01: Document state machine — every business document (ticket, invoice, purchase order, inventory session, delivery note) has a `type`, a `state`, and a list of allowed transitions filtered by the current user's capabilities. The backend returns only the transitions the user can trigger. Flutter renders action buttons dynamically — no permission logic client-side. Examples: ticket_caisse → [normalize_to_invoice] → facture_normalisée; client_order → [validate] → delivery_note → [invoice] → facture. *(Architecture H1 — Implementation V2)*

FR-WORKFLOW-02: Workflow step ownership (SOD — Segregation of Duties) — each workflow instance is decomposed into steps (`WorkflowStep`) with distinct assignment per step. Example: inventory_session = step 1 "create_session" (manager) + step 2 "execute_count" (operator) + step 3 "validate_discrepancies" (director). Each step has status (pending/in_progress/done), assignee (userId or roleId), optional deadline. The person who initiates ≠ the person who executes ≠ the person who validates. This pattern generalizes the 3-level session closing already identified for Blandine. *(Architecture H1 — Implementation V2)*

**SDUI Capability Filtering (FR-SDUI-01)**
FR-SDUI-01: Permission filtering on SDUI components — each component in the layout JSON (button, widget, section, list) can carry a `requires: "capability.code"` field. Before returning the layout to the client, the backend evaluates the connected user's capabilities and silently removes any component whose capability is absent. The Flutter client does not know why a button is absent — it simply does not render it. Examples: "Normalize to invoice" button → `requires: "billing.normalize_to_invoice"`; "Cash balance" widget → `requires: "session.view_cash_balance"`. Fundamental rule: business logic (calculations, transitions, validations) stays in NestJS — never in the SDUI. SDUI says what to display. Backend says what to authorize. *(Architecture H1 — Implementation V2)*
