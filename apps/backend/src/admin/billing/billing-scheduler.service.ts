...
export class BillingSchedulerService {
  ...
  @Cron('0 2 * * *', { name: 'billing-status-check', timeZone: 'UTC' })
  ...
  async checkOverdueSuspension() {
    const suspensionDays = parseInt(process.env.BILLING_SUSPENSION_DAYS ?? '30', 10);
    const cutoff = new Date(Date.now() - suspensionDays * 24 * 60 * 60 * 1000);
    ...
    await (this.prisma as any).billingEvent.createMany({
      data: overdue.map((t) => ({
        ...
        amount: 0,
        status: 'overdue',
        description: `Suspension automatique — impayé > ${suspensionDays}j`,
      })),
    });
