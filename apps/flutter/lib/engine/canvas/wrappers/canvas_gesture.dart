...
class CanvasGesture extends StatelessWidget {
  ...
  static Widget wrap(ComponentConfig config, Widget child) {
    ...
    if (gestures.containsKey('swipe_left')) {
      wrapped = Dismissible(key: config.id != null ? ValueKey(config.id) : UniqueKey(), direction: DismissDirection.endToStart, background: Container(color: colors['warning']), onDismissed: (_) {}, child: wrapped);
    }
    if (gestures.containsKey('swipe_right')) {
      wrapped = Dismissible(key: config.id != null ? ValueKey('${config.id}_r') : UniqueKey(), direction: DismissDirection.startToEnd, background: Container(color: colors['success']), onDismissed: (_) {}, child: wrapped);
    }
...
const colors = {'warning': Colors.orange, 'success': Colors.green};
