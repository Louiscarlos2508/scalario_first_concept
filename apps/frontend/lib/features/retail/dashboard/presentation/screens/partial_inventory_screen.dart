...
class _PartialInventoryScreenState
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(_phase == _Phase.selection
            ? 'Inventaire partiel — Sélection'
            : 'Feuille de comptage'),
        leading: _phase == _Phase.counting
            ? IconButton(
      ...
      child: Column(
        ...
        children: [
          ...products.map((p) {
            ...
            // Variance signal: only shown when both values are known
            bool? isOk;
          ...
          // Global reason field — only when at least one écart
          if (hasAnyVariance) ...[
            const SizedBox(height: 8),
            TextFormField(
